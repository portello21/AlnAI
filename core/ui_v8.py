...
.rog-brand{display:flex;align-items:center;gap:11px;padding:4px 4px 17px}.rog-logo{width:39px;height:39px;border-radius:12px;display:grid;place-items:center;font-weight:900;background:linear-gradient(135deg,#684bf0,#9b82ff);box-shadow:0 8px 28px rgba(118,87,245,.27)}.rog-brand-title{font-weight:850;font-size:16px}.rog-brand-sub{font-size:9px;color:var(--rog-muted);letter-spacing:.7px;margin-top:2px}.rog-profile{padding:10px 12px;border-radius:13px;border:1px solid var(--rog-line);background:rgba(255,255,255,.025);margin:2px 2px 17px}.rog-profile strong{font-size:11px}.rog-profile span{display:block;color:var(--rog-success);font-size:8px;margin-top:3px}.rog-section{color:#697386;font-size:8px;text-transform:uppercase;letter-spacing:1.2px;font-weight:850;margin:14px 6px 8px}.rog-security{margin:12px 2px 4px;padding:9px 10px;border:1px solid var(--rog-line);border-radius:11px;background:rgba(255,255,255,.018);font-size:8px;line-height:1.45;color:#788497}
...
def render_security_note(profile: str) -> None:
    label = PROFILE_LABELS.get(str(profile or "").lower(), str(profile or "").title())
    st.markdown(f'<div class="rog-security">🔒 {html.escape(label)} possui workspace privado. Somente Allan e Beatriz podem compartilhar o namespace financeiro, exclusivamente no agente Finanças.</div>', unsafe_allow_html=True)


def render_agent_header(agent_id: str) -> None:
    icon, name, desc = AGENT_META.get(agent_id, AGENT_META["orchestrator"])
    st.markdown(f'<div class="rog-hero"><div class="rog-hero-row"><div class="rog-agent-icon">{html.escape(icon)}</div><div><div class="rog-agent-name">{html.escape(name)}</div><div class="rog-agent-desc">{html.escape(desc)}</div></div><div class="rog-online">● Online</div></div></div>', unsafe_allow_html=True)
...
def render_welcome(agent_id: str, profile: str) -> None:
    ...
    st.markdown(f'<div class="rog-welcome"><div class="rog-welcome-orb">{html.escape(icon)}</div><h2>Como posso ajudar, {html.escape(person)}?</h2><p>Você está no <strong>{html.escape(name)}</strong>. Envie uma mensagem, anexe um arquivo ou escolha outro especialista na barra lateral.</p></div>', unsafe_allow_html=True)
